...
PTB (Price To Beat) mode:
    PTB_MODE = "ask"   -> har naye window pe terminal manual input maangega (timeout ke saath).
                          Kuch type kiya (jaise 62103.19) -> woh use hoga as btc_open.
                          Khali Enter / timeout -> auto (Binance) fallback.
    PTB_MODE = "manual"-> hamesha manual input maangega, koi timeout fallback nahi (jab tak input na de, ruka rahega).
    PTB_MODE = "auto"  -> kabhi nahi poochega, seedha Binance se le lega (pehle wala behavior).
...
Logic per 5-min window:
  ...
  Window close : compare final BTC price to window-open BTC price (PTB baseline) -> actual outcome (Up/Down).
...
# PTB source: "ask" (prompt with timeout, fallback auto), "manual" (always prompt, no fallback),
# "auto" (never prompt, Binance only)
PTB_MODE = "ask"
PTB_PROMPT_TIMEOUT = 12   # seconds to wait for manual PTB input before falling back to auto
...
def prompt_manual_ptb(timeout):
    """
    Non-blocking-ish prompt: waits up to `timeout` seconds on stdin.
    Returns float if user typed a valid number, else None (caller decides fallback).
    Windows has no select() on stdin, so this only works cleanly on Linux/Mac (VPS = fine).
    """
    print(f"  >> Price To Beat manually daal (Polymarket se copy-paste), {timeout}s ke andar "
          f"[Enter khali chhod = auto]: ", end="", flush=True)
    try:
    ...
    if ready:
        ...
        if line:
            try:
                ...
                print(f"  [warn] '{line}' number nahi hai, auto fallback ho raha hai.")
                return None
    print("  (timeout / khali — auto fallback)")
    return None


def resolve_btc_open():
    """Decide btc_open (PTB baseline) based on PTB_MODE."""
    if PTB_MODE == "auto":
        return fetch_btc_price(), "auto"

    if PTB_MODE == "manual":
        while True:
            val = prompt_manual_ptb(timeout=3600)  # effectively wait indefinitely, retry loop
            if val is not None:
                return val, "manual"
            # manual mode with empty input -> ask again (no silent auto fallback)
            print("  [manual mode] khali chhod nahi sakta, phir se daal.")

    # PTB_MODE == "ask" (default): prompt with timeout, fallback to auto
    val = prompt_manual_ptb(timeout=PTB_PROMPT_TIMEOUT)
    if val is not None:
        return val, "manual"
    return fetch_btc_price(), "auto"
...
# ---------- per-window state ----------
class WindowState:
    def __init__(self, window_ts, up_id, down_id, slug, btc_open, ptb_source):
...
def main():
    ...
    print(f"PTB_MODE = {PTB_MODE!r} (change at top of script: 'ask' / 'manual' / 'auto')\n")
    ...
    while True:
        ...
        if window_ts != current_window_ts:
            ...
            btc_open, ptb_source = resolve_btc_open()
            print(f"\n=== New window {window_ts} ({slug}) | Price To Beat={btc_open} (source={ptb_source}) ===")
            state = WindowState(window_ts, up_id, down_id, slug, btc_open, ptb_source)
        ...
        with open(tpath, "a", newline="") as f:
            csv.writer(f).writerow([label, int(now), up_bid, up_ask, down_bid, down_ask,
                                     btc_price, ptb, state.ptb_source])
        ...
        print(f"{label} price_to_beat={state.btc_open}({state.ptb_source}) btc={btc_price} "
              f"ptb_diff={ptb} up_ask={up_ask} down_ask={down_ask}{status_bit}")
